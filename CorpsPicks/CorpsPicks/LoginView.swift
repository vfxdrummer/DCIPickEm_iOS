...
//    self.title = Constants.contestTitle
    ...
    loginViewModel?.loadEvents()
    
    self.refreshControl!.addTarget(self, action: #selector(LoginView.refresh(_:)), for: UIControlEvents.valueChanged)
  }
  ...
  /**
   ...
   Called by loginViewModel on Load Complete [Handles Events Pulled from Remote]
  ...
  override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
    return loginViewModel!.events.count
  }
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    cell.LoginView = self
    cell.load(indexPath.row, event: loginViewModel!.events[indexPath.row])
    return cell
  }
  
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    let cell = tableView.cellForRow(at: indexPath) as! EventRow
    if let event = loginViewModel?.events[indexPath.row] as Event? {
